" Turn off ycm for python files; use syntastic instead
"let g:ycm_auto_trigger = 0


" Fix Python Path (for YCM)
let g:ycm_path_to_python_interpreter=system("which python | sed 's/^/\"/g' | sed 's/$/\"/g'")

" Wrapping and tabs.
set ts=4 sw=4 sta et sts=4 ai

" More syntax highlighting. Syntax highlighting specs are 
" now moved to the bottom
"let python_highlight_all = 1

" For creating split screens with python shells
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
" Map opening ConqueTermVsplit ipython to Ctrl+i
map <A-i> :ConqueTermVSplit ipython<CR>
" Map some things to allow me to run python code to Conqueshell
...
" I do not currently need it because I have the evaluation using
" Conqueterm on ipython
"map <C-y> :py3 EvaluateCurrentRange()<CR>

" Use F7/Shift-F7 to add/remove a breakpoint (bpdb.set_trace)
" Totally cool.
python3 << EOF
...
def SetBreakpoint():
    ...
    vim.current.buffer.append(
        "%(space)sbpdb.set_trace()  %(startmark)s Breakpoint %(endmark)s" %
        {'space':strWhite, 'startmark': '# ' + '-'*30, 'endmark': '-'*30 + ' #'}, nLine - 1)

    for strLine in vim.current.buffer:
        if strLine == "import bpdb":
            break
    else:
        vim.current.buffer.append( 'import bpdb', 0)
...
def RemoveBreakpoints():
    ...
    nLine = 1
    for strLine in vim.current.buffer:
        if strLine == "import bpdb" or strLine.lstrip()[:16] == "bpdb.set_trace()":
            nLines.append( nLine)
        nLine += 1
